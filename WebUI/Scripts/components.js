$(document) .ready(function () {
  ...
  if ($('body') .hasClass('demo-maps')) {
    ...
    $mapZoom.on('mousewheel', function (e) {
      return e.deltaY > 0 ? $mapZoom.trigger('zoom', $mapZoom.data('zoomLevel') + 1)  : $mapZoom.trigger('zoom', $mapZoom.data('zoomLevel') - 1),
      !1;
    }),
    $('#focus-paris') .on('click', function () {
      var e = $.fn.mapael.maps.france_departments.getCoords(48.114167, 2.3444);
      $mapZoom.trigger('zoom', [
        10,
        e.x,
        e.y
      ]);
  ...
  if ($('.data-us-map') .length > 0 && $('.data-us-map') .mapael({
    ...
    $('#btn-quick-event') .click(function () {
      ...
      n = 'Untitled Event';
      '' != $('#quick-event-name') .val() && (n = $('#quick-event-name') .val()),
      t.title = n,
      t.start = a,
      t.color = $colorPicker.val(),
