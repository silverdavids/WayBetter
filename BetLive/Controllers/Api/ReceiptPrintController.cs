...
namespace BetLive.Controllers.Api
{
    public class ReceiptPrintController : ApiController
    {
        ...
        public async Task<IHttpActionResult> ReceiveReceipt([FromBody]Receipt1 receipts)
        {
            ...
            // IEnumerable<string> headerValues =RequestContext.Principal..PrincipalIdentityrequest.Headers.GetValues("MyCustomID");
           // var id = headerValues.FirstOrDefault();
            //var identity = (ClaimsIdentity)User.Identity;
            //Claim claims = identity.FindFirst("UserName");
            //var un = RequestContext.Principal.Identity.Name;
           // var userName = RequestContext.Principal.Identity.Name;//context.Request.User.Identity.Name;
           // ClaimsPrincipal principle = Request.GetRequestContext().Principal as ClaimsPrincipal;
           // var userName = principle.Claims.Where(c => c.Type == "sub").SingleOrDefault().Value;
           
           // var user = this.User.Identity; 
            var userName = receipts.UserName;
            var account = BetDatabase.Accounts.SingleOrDefault(x => x.UserId == userName);
            //if (account == null)
            //{
            //    account = new Account
            //    {
            ...
            if (account == null)
            { 
             response="Invalid user name or password please sign in or contact tour systems admin";
            ...
            {
                UserId = /*"TellerTest",*//*User.Identity.Name*/ userName,
